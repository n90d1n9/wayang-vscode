...
export class Messages {
  render(state: any): string {
    ...
    return `
        <div class="chat-messages" id="chatMessages">
            ...
            ${isStreaming ? `<div class="typing-indicator">🤖 Assistant is typing<span class="dots">...</span></div>` : ''}
        </div>
    `;
}
  ...
  private renderMessage(message: ChatMessage): string {
    ...
    return `
      <div class="message ${message.type} ${message.loading ? "loading" : ""} ${
      message.error ? "error" : ""
    } ${message.pinned ? "pinned" : ""} ${
      message.edited ? "edited" : ""
    }" id="message-${message.id}">
          <div class="message-header">
              <span>${this.getMessageIcon(message.type)} ${
      this.getMessageRole(message.type)
    }${message.mode ? ` (${message.mode})` : ""}</span>
  ...
  private getMessageActions(message: ChatMessage): string {
    ...
    if (message.type === "assistant") {
      return `
        ...
        <button class="message-action" onclick="pinMessage('${message.id}')" title="Pin">${
        message.pinned ? "📌" : "📍"
      }</button>
  ...
  private formatMessageContent(content: string): string {
    ...
    return content
      ...
      .replace(/\*(.*?)\*/g, "<em>$1</em>")
      .replace(/```(\w+)?\n([\s\S]*?)\n```/g, (match, lang, code) => {
        return `<pre data-language="${lang || "code"}"><code>${
  ...
  private addConfidenceIndicator(confidence: number): string {
    const confidenceIcon = confidence > 0.8
      ...
      ? "🟡"
      : "🔴";
    const confidenceText = confidence > 0.8
      ? "High"
      : confidence > 0.6
      ? "Medium"
      : "Low";
    ...
    return `
      <div class="confidence-indicator">
          ${confidenceIcon} ${confidenceText} Confidence (${
    ...
    return `
      <div class="code-suggestion fade-in">
          <div class="code-suggestion-header">
              <span>💡 ${
      suggestion.title || suggestion.file || "Code Suggestion"
    }</span>
              <div>
                  <button class="apply-code-btn" onclick="applyCodeSuggestion('${
      this.escapeHtml(suggestion.code)
    }', '${suggestion.file || ""}')">
                      Apply
                  </button>
                  <button class="message-action" onclick="previewCode('${
      this.escapeHtml(suggestion.code)
    }', '${suggestion.file || ""}')">
                      Preview
                  </button>
          ...
          <div class="code-suggestion-content">
              <pre data-language="${suggestion.language || "code"}"><code>${
      this.escapeHtml(suggestion.code)
    }</code></pre>
              ${
